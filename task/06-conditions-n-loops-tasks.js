...
function getDigitalRoot(num) {
    ...
    return num % 10 === num ? num : getDigitalRoot(sum(num));
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < path.length; ++i){
      ...
      for(let j = 0; j < pathes.length; ++j){
          if (i > pathes[j].length ||path[i] != pathes[j][i]){
              bool = false;
              break;
          }
...
function evaluateTicTacToePosition(board) {
   for (var i = 0; i < 3; ++i){
      if (board[i][0] == board[i][1] && board[i][2] == board[i][1] && board[i][0] !== undefined)
          return board[i][0];
          
      if (board[0][i] == board[1][i] && board[2][i] == board[1][i] && board[0][i] !== undefined)
          return board[0][i];
   }
   if (board[0][0] == board[1][1] && board[1][1] == board[2][2] && board[0][0] !== undefined)
      return board[0][0];
     
   if (board[0][2] == board[1][1] && board[1][1] == board[2][0] && board[2][0] !== undefined)
      return board[0][2];
     
   for (var i = 0; i < 3; ++i)
      if (board[i][0] == 0 || board[i][2] == 0 || board[i][1] == undefined)
         return undefined;
   return undefined;
